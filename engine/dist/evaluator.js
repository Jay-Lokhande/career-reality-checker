...
export class Evaluator {
    ...
    evaluate(input) {
        ...
        const overallScore = Math.round(experienceScore * 0.3 +
            skillScore * 0.3 +
            educationScore * 0.2 +
            timelineScore * 0.2);
        ...
        const probabilityBands = this.calculateProbabilityBands(profile, goal, experienceScore, skillScore, educationScore, timelineScore, overallScore);
        // Step 4: Generate warnings for unrealistic expectations
        const warnings = this.generateWarnings(profile, goal, {
            experienceScore,
            skillScore,
            educationScore,
            timelineScore,
        ...
        const recommendations = this.generateRecommendations(profile, goal, experienceScore, skillScore, educationScore, timelineScore, skillGaps);
     ...
     */
    calculateExperienceScore(profile, goal) {
     ...
     */
    calculateEducationScore(profile, goal) {
        ...
        if (education.field && goal.targetIndustry) {
            ...
            if ((fieldLower.includes('computer') || fieldLower.includes('software') || fieldLower.includes('tech')) &&
                (industryLower.includes('tech') || industryLower.includes('software') || industryLower.includes('it'))) {
                fieldBonus = 20;
            }
            else if ((fieldLower.includes('business') || fieldLower.includes('management') || fieldLower.includes('finance')) &&
     ...
     */
    calculateTimelineScore(profile, goal) {
     ...
     */
    estimateExperienceGap(profile, goal) {
        const relevantYears = profile.experience.relevantYears;
        const roleTitle = goal.targetRole.toLowerCase();
     ...
     * - Required actions
     */
    calculateProbabilityBands(profile, goal, experienceScore, skillScore, educationScore, timelineScore, overallScore) {
        ...
        const isCareerChange = profile.experience.relevantYears < 1;
        const baseMonths = Math.max(2, experienceGap * 12 + totalSkillMonths + (isCareerChange ? 6 : 0));
     ...
     */
    calculateLikelihood(overallScore, band) {
        if (band === 'average') {
     ...
     */
    calculateSacrifices(profile, goal, band, requiredHours) {
     ...
     */
    getContributingFactors(overallScore, experienceScore, skillScore, educationScore, band) {
        const factors = [];
        if (band === 'best') {
            if (overallScore >= 80) {
     ...
     */
    getRequiredActions(profile, goal, experienceScore, skillScore, band) {
        const actions = [];
     ...
     */
    generateWarnings(profile, goal, scores, probabilityBands) {
        const warnings = [];
        // Timeline warning
        ...
        if (expectationWarning)
            warnings.push(expectationWarning);
     ...
     */
    identifySkillGaps(profile, goal) {
        const gaps = [];
     ...
     */
    generateRecommendations(profile, goal, experienceScore, skillScore, educationScore, timelineScore, skillGaps) {
     ...
     */
    checkTimeAvailabilityWarning(profile, goal, probabilityBands) {
     ...
     */
    checkSkillMismatchRisksWarning(profile, goal, skillScore) {
        const warnings = [];
        ...
        for (const requirement of matchingScenario.skillRequirements) {
            if (!requirement.isCritical)
                continue; // Only check critical skills
            // Find matching skill in user's profile
            const userSkill = profile.skills.find(skill => skill.name.toLowerCase().includes(requirement.skillName.toLowerCase()) ||
